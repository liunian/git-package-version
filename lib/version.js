...
function test(dir) {
    ...
    if (notMatch) {
        console.error('npm version(' + versions.npmVersion +
            ') is not matches daily version (' + versions.gitVersion + ')');
        updateNpmVersion(versions.gitVersion);
    } else {
        console.log('version: ' + versions.npmVersion + ', ok');
    }
}
...
function updateNpmVersion(version) {
    ...
    var answer = readlineSync.keyInYN(
        'update npm version to ' + version + '？', {
        defaultInput: 'Y'
    });
